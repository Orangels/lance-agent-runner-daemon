...
export function createWorkspacesRouter(dependencies: CreateWorkspacesRouterDependencies): Router {
  ...
  router.post('/', auth, (request, response, next) => {
    try {
      ...
      const workspace = dependencies.workspaceService.createOrGetWorkspace({
        clientId: client.id,
        profile,
  ...
  router.post('/:workspaceId/prepare', auth, (request, response, next) => {
    try {
      const client = (request as AuthenticatedRequest).client;
      ...
      response.json(
        dependencies.workspaceService.prepareWorkspaceFiles({
          clientId: client.id,
          isAdmin: client.isAdmin,
          profile,
