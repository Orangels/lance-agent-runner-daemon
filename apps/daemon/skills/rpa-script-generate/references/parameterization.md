...
参数候选确认必须优先使用 AskQuestion；如果当前 Claude Code 环境没有真实 AskQuestion 工具，则输出等价的 `<question-form id="rpa-parameterization">` 结构化表单，由 RPA Web 渲染后把 `[form answers — rpa-parameterization]` 作为下一轮普通用户消息回传。Claude Code 收到答案后再更新 DSL 和脚本。
