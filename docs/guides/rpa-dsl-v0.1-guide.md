...
DSL 不是完整的业务脚本语言。复杂业务判断、数据清洗、JSON 生成、异常兜底、下载文件写入、审计日志等逻辑应放在 `flow.hardened.py` 中实现，并在报告中说明。
...
- 本地 JSON、下载文件、日志、trace、截图等不是页面元素操作，优先放在 `flow.hardened.py` 中实现。
...
- `download_exists`：执行目录中应存在指定产物或下载文件。
