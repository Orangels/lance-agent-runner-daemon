...
```text
run created
  -> 初始化 user message 和第一条 assistant message draft
...
每个 Claude parser event
  -> consume(event)
  -> 遇到新的 Claude assistant message 边界时创建下一条 assistant message
  -> 更新当前 assistant message 的 content/events
  -> 按 500ms 左右节流保存
...
第一版中 batch run 和 chat run 都采用 user message + assistant message draft 模型；运行中如果 Claude Code 在同一个 run 内产生多轮 assistant message，daemon 会为后续 assistant message 继续追加 `run_messages` 行。业务端不传 `conversationId` 时，daemon 为每个 workspace 创建或复用一条默认 conversation，并把本次 run 的 user/assistant messages 归入该默认线程。
...
这是最需要复用 lanceDesign 语义的表。lanceDesign 的 `messages` 保存的是前端翻译/合并后的聊天消息和 agent events，不是每个原始 SSE chunk。新 daemon 保存翻译/合并后的 `content + thinking_content + events_json`，区别是保存动作由 daemon 后端 accumulator 触发，而不是依赖前端消费 SSE。一个 run 至少有 user message 和第一条 assistant draft；同一 run 内每个后续 Claude assistant message 边界会追加一条新的 assistant message。
