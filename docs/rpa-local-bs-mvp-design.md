...
后续如果需要更强隔离或更高并发，可以把 executor 拆成独立服务或 worker。
...
- **RPA Web**：解析 assistant 文本中的 `<question-form>`，渲染 radio / checkbox / select / text / textarea，用户提交后生成下一轮普通消息。
...
executor 执行时向 RPA Web BFF 发事件，前端基于 DSL 展示当前步骤：

```json
{
  "type": "step_started",
  "stepId": "step_003",
  "screenshot": "runs/run_123/steps/step_003.png",
  "highlight": {
...
```json
{
  ...
  "requirements": {
    ...
    "browser": "chromium",
    "manualIntervention": ["captcha", "ca_usbkey"]
  }
}
```
...
- 导出 `flow.dsl.json`、`flow.hardened.py`、`config.example.json`、`hardening-report.md` 和 `manifest.json`。
- 可选导出脱敏后的样例截图和验证报告。
...
1. MVP 优先做自然语言生成，还是 codegen 上传加固。
2. 基于 DSL 草案固化 `flow.dsl.json` JSON Schema。
3. executor 的最小 Python/Playwright 运行约定。
4. RPA Web 第一版页面结构。
